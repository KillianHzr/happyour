...
export default function MainPagerScreen() {
  ...
  const fetchAllData = useCallback(async () => {
    ...
    try {
      ...
      const now = new Date();
      const inRevealWindow = now >= prevRevealDate && now < (new Date(prevRevealDate.getTime() + 16 * 60 * 60 * 1000));
      const photoStart = inRevealWindow ? new Date(prevRevealDate.getTime() - 7 * 24 * 60 * 60 * 1000) : prevRevealDate;
