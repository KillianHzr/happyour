...
export const GroupIcon = ({ color = colors.white }) => (
...
export const GroupAddIcon = ({ color = colors.white }) => (
...
export const SendIcon = ({ color = colors.black }) => (
...
export const FlashIcon = ({ mode }: { mode: FlashMode }) => {
  const color = mode === "off" ? "rgba(255,255,255,0.4)" : colors.white;
  return (
    <Svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <Path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z" />
      {mode === "off" && <Path d="M2 2l20 20" />}
      {mode === "auto" && <Circle cx="18" cy="6" r="3" stroke={colors.white} strokeWidth="1" />}
    </Svg>
  );
};
