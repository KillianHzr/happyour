...
function CameraPageInner({ groupId, userId, isActive, allGroups, onScrollLock, onCaptureSent }: Props) {
  ...
  const cancelCaptionAudioRecordingRef = useRef(cancelCaptionAudioRecording);
  ...
  const confirmUpload = (groupIds: string[]) => {
    ...
    groupIds.forEach((gId, i) => {
      ...
      if (activeChallenge !== null) {
        ...
        startUpload(fileName, fileUri, contentType, dbData, secondFile, captionAudioFile, slot1.waveform, slot1.captionWaveform);
      }
    });
    onCaptureSent?.();
    resetAll();
                            ...
                            >
                              <View style={styles.captionIconContainer}>
                                ...
                                {isCaptionRecording ? (
                                  ...
                                  <MicIcon color={colors.white} size={20} />
                                )}
                              </View>
                            </View>
                          </>
...
function TrashIcon({ color = colors.white }: { color?: string }) {
  return (
...
const styles = StyleSheet.create({
  ...
  previewContent: { position: "absolute", left: 12, right: 12 },
  previewNoteBox: { flex: 1, paddingVertical: 12 },
  previewNoteText: { color: colors.white, fontSize: typography.size.sm, fontFamily: typography.family.semibold },
  addNoteBtn: { flex: 1, flexDirection: "row", alignItems: "center", gap: 10, paddingVertical: 12 },
  ...
  combinedInputContainer: {
    ...
    borderRadius: radii.lg, // var(--sds-size-radius-400) -> 16
    paddingHorizontal: spacing.md, // var(--sds-size-space-300) -> 12
    height: 48, // Fixed height from CSS
    gap: 0, // var(--sds-size-space-0)
    overflow: "hidden", // necessary for blur effect approximation in React Native
  },
  recordingContainer: {
    flex: 1,
    flexDirection: "row",
