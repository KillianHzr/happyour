...
export default function CommentModalIOS(props: CommentModalProps) {
...
function CommentModalContent({ 
  ...
  useEffect(() => {
    console.log("[CommentModalIOS] Received reactions count:", reactions.length, "for photo:", photoId);
    reactions.forEach((r, idx) => {
      console.log(`[CommentModalIOS] Reaction #${idx} - ID: ${r.id}, User: ${r.username}, StickerID: ${r.sticker_id}, isEmoji: ${isEmoji(r.sticker_id)}`);
    });
  }, [reactions, photoId]);
  ...
  const markAsSeen = useCallback(async () => {
    ...
    try {
      ...
      console.error("[CommentModalIOS] Error marking as seen:", e);
    }
  ...
  const handleLongPressDelete = useCallback((commentId: string, anchorY: number) => {
    if (modalContainerRef.current) {
      ...
      setDeletePopup({ commentId, anchorY, sheetTopY: SCREEN_HEIGHT - currentModalHeight });
    }
  }, [currentModalHeight]);
...
function CommentModalBody({
  ...
  // Filter out current-user from suggestions (done in CommentInput, but kept here for popup)
  const handleSuggestionSelect = useCallback((member: GroupMember) => {
    // Find the last @ in the content and replace the partial mention
